...
def dijkstra(graph, start, goal):
    ...
    while queue:
        # Encuentra el nodo con la distancia mas corta en la fila
        current_node = min(queue, key=lambda node: shortest_distance[node])
        ...
        for neighbor in graph[current_node]:
            # Calcula la nueva disttancia desde el comienzo hasta el nodo vecino
            distance = shortest_distance[current_node] + graph[current_node][neighbor].get('weight', 1)
        ...
        # Resalta el nodo que se esta explorando
